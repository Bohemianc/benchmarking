...
import json
import random
from embedding import get_type as gt
import sys
...
class Config(object):
        ...
        with self.graph.as_default():
            with self.sess.as_default():
                ...
                if self.test_triple_classification:
                    ...
                    res_pos = self.test_step(
                        self.test_pos_h, self.test_pos_t, self.test_pos_r
                    )
                    res_neg = self.test_step(
                        self.test_neg_h, self.test_neg_t, self.test_neg_r
                    )
    ...
    def get_test_file(self):
        ...
        with open(self.in_path + "train2id.txt", "r") as f:
            ...
            # create test file
            file = open(os.path.join(self.in_path, "test2id.txt"), "w")
            file.write(str(test_num) + "\n")
            for index in range(test_num):
                file.write(
                    (data_total[sample_list[index]][0])
                    + " "
            ...
            for index in range(valid_num):
                file.write(
                    (data_total[sample_list[index + test_num]][0])
                    + " "
