...
from embedding.models import Model


class TransE(Model.Model):
    r"""
    ...
    def loss_def(self):
        ...
        p_score = tf.reduce_sum(
            tf.reduce_mean(_p_score, 1, keep_dims=False), 1, keep_dims=True
        )
        n_score = tf.reduce_sum(
            tf.reduce_mean(_n_score, 1, keep_dims=False), 1, keep_dims=True
        )
    ...
    def predict_def(self):
        ...
        self.predict = tf.reduce_mean(
            self._calc(predict_h_e, predict_t_e, predict_r_e), 1, keep_dims=False
        )
